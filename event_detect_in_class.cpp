...
void Event_detect::recieve_data()
{
	...
	vector<vector<Vehicleincident_Detection>> vehs_test = { {Vehicleincident_Detection(1,1,1,2, 5, "light", -1.7,205.0, 1.3, 0, 4, 1.8),
												  Vehicleincident_Detection(1,1,2,2, 5, "light", -1.7,203.5, 1.3, 0, 4, 1.8)},
												 {Vehicleincident_Detection(2,1,1,2, 5, "light", -1.7,184.0, 1.3, 0, 4, 1.8),
												  Vehicleincident_Detection(2,1,2,2, 5, "light", -1.7,180.0, 1.3, 0, 4, 1.8)},
												 {Vehicleincident_Detection(5,1,1,2, 5, "light", -1.7,75.0 , 1.3, 0, 4, 1.8),
												  Vehicleincident_Detection(5,1,2,2, 5, "light", -1.7,70.0 , 1.3, 0, 4, 1.8)},
		                                         {Vehicleincident_Detection(9,1,1,2,5, "light",-1.7,42.0 , 1.3, 0, 4, 1.8),
												  Vehicleincident_Detection(9,1,2,2,5, "light", -1.7,41.0 , 1.3, 0, 4, 1.8)},
		                                         {Vehicleincident_Detection(10,2,1,2,5, "light", -1.7,31.0 , 1.3, 0, 4, 1.8),
												  Vehicleincident_Detection(10,2,2,2,5, "light", -1.7, 30.5, 10, 0, 4, 1.8)}
	};
	...
	for (int i = 0; i < vehs_test.size(); i++) {
		...
		for (int j = 0; j < vehs_test[i].size(); j++) {
			...
			if (vehs_test[i][j].lane_Num > 10 && vehs_test[i][j].lane_Num < 30) {
	...
	if (fmod(time_stamp, time_Interval) == 0.0 && !flag) {
		...
		for (auto it = intersection_Devices_Detect.begin(); it != intersection_Devices_Detect.end(); it++) {
			...
			for (int i = 0; i < it->second.volume_test.lane_Code.size(); i++) {
				...
				//printf("eventID:%d, eventType:%d, description:{车道ID:%d, 车道流量:%f}, eventSource:%d \n",eventID,eventType,it->second.volume_test.lane_Code[i], it->second.volume_test.lanes_Volume[it->second.volume_test.lane_Code[i]],object_event_source); 
				//printf("eventID:%d, eventType:%d, description:{车道ID:%d, 平均车头时距:%f}, eventSource:%d \n", eventID, eventType, it->second.capacity_test.lane_Code[i],it->second.capacity_test.ave_Time_Headway[it->second.capacity_test.lane_Code[i]], object_event_source); 
				//printf("eventID:%d, eventType:%d, description:{车道ID:%d, 车道排队长度:%f}, eventSource:%d \n", eventID, eventType, it->second.max_queue_test.lane_Code[i],it->second.max_queue_test.lanes_Queue_Length[it->second.max_queue_test.lane_Code[i]], object_event_source); 
			}
			//printf("eventID:%d, eventType:%d, description:{平均车头间距:%f}, eventSource:%d \n", eventID, eventType, it->second.max_queue_test.ave_Space_Headway, object_event_source); 

			it->second.speed_test.caculation_Index();                                                //计算空间平均速度
			//printf("eventID:%d, eventType:%d, description:{平均空间速度:%f}, eventSource:%d \n", eventID, eventType, it->second.speed_test.ave_Space_Speed, object_event_source); 
